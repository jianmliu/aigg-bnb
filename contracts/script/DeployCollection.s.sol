...
/// lists it. The genesis set is read from a published file (GENESIS_FILE, default genesis-v1.json),
...
///   TREASURY (default: a new TreasuryRouter owned by TREASURY_OWNER and forwarding to TREASURY_DESTINATION, both the broadcaster), LINEAGE (default: none -- the legacy `register`), BASE_MALE (default: manifest baseMale, or zero)
...
contract DeployCollection is Script {
    ...
    function run() external returns (FlyCollection c, bool listed) {
        string memory genesisFile = vm.envOr("GENESIS_FILE", string("genesis-v1.json"));
        string memory g = vm.readFile(string.concat(vm.projectRoot(), "/../flybnb/genesis/", genesisFile));
        bytes32 root = g.readBytes32(".root"); uint32 size = uint32(g.readUint(".size")); bytes32 baseFemale = g.readBytes32(".baseModelId");
        bytes32 baseMale = vm.keyExistsJson(g, ".baseMale") ? g.readBytes32(".baseMale") : bytes32(0);
        baseMale = vm.envOr("BASE_MALE", baseMale);
        if (g.readUint(".sexes.male") > 0) {
            require(baseMale == g.readBytes32(".baseMale"), "male base differs from genesis");
            require(vm.envOr("BASE_MEP_MALE", bytes32(0)) != bytes32(0), "male base MEP required");
        }
        ...
        c = new FlyCollection(baseFemale, baseMale, root, size, price, bond, breedFee, bounty, treasury,
        ...
        if (whitelist != address(0)) { try CollectionWhitelist(whitelist).add(address(c), "FlyBnB research Founder collection") { listed = true; } catch {} }
        vm.stopBroadcast();
