...
interface ITokenRoyaltyReceiver { function MARKET() external view returns(address); function supportsTokenRoyalties() external view returns(bool); function onTokenRoyalty(bytes32,address,uint256) external; }

interface IAssetDisputeOpener { function openDispute(bytes32 taskId, address a, address b) external; }
...
contract MultiAssetTaskMarket is ITaskMarket {
    mapping(bytes32=>bool) public disputeResolved;
    mapping(bytes32=>bool) public tokenRoyaltyCallback;
    ...
    event TokenAccepted(address indexed host,address indexed token,bool accepted);
    ...
    function setTokenAllowed(address token,bool allowed) external {require(msg.sender==owner&&token!=address(0)&&token.code.length>0,"owner/token");tokenAllowed[token]=allowed;emit TokenAllowed(token,allowed);}
    function setTokenBeneficiaryAllowed(address beneficiary,bool allowed) external {require(msg.sender==owner,"owner");if(allowed)require(ITokenRoyaltyReceiver(beneficiary).supportsTokenRoyalties()&&ITokenRoyaltyReceiver(beneficiary).MARKET()==address(this),"token receiver");tokenBeneficiaryAllowed[beneficiary]=allowed;}
    function setAcceptedToken(address token,bool accepted) external {require(token!=address(0),"native default");acceptedToken[msg.sender][token]=accepted;emit TokenAccepted(msg.sender,token,accepted);}
    function tokenTaskId(Task calldata t,address token,bytes32 nonce,uint32 runs,address client) public view returns(bytes32){
        bytes32 base=runs==0?PorwMeshHash.taskId(t,nonce):PorwMeshHash.batchId(t,runs,nonce);
        return keccak256(abi.encode(keccak256("AIGG_ASSET_TASK_V1"),block.chainid,address(this),token,client,base));
    }
    function postTokenTask(Task calldata t,address token,bytes32 nonce) external guard returns(bytes32 id){return _postToken(t,token,nonce,0);}
    function postTokenBatch(Task calldata t,address token,uint32 runs,bytes32 nonce) external guard returns(bytes32 id){
        require(runs>=2&&runs<=MAX_RUNS&&t.stimulusSeed==0,"batch");
        require(meps.lifWeightUnit(meps.getMEP(t.mepId).execKind)!=0,"int-lif only");
        id=_postToken(t,token,nonce,runs);batchRuns[id]=runs;emit BatchPosted(id,runs,t.initStateRoot);
    }
    function _postToken(Task calldata t,address token,bytes32 nonce,uint32 runs) internal returns(bytes32 id){
        require(tokenAllowed[token]&&t.fee>0,"token/fee");
        (address ben,uint16 bps)=meps.termsOf(t.mepId);
        require(bps==0||ben.code.length==0||tokenBeneficiaryAllowed[ben],"token beneficiary");
        id=tokenTaskId(t,token,nonce,runs,msg.sender);require(!tasks[id].exists,"posted");
        paymentToken[id]=token;tokenRoyaltyCallback[id]=ben.code.length>0&&bps>0;TokenTransfer.pull(token,msg.sender,address(this),t.fee);_post(t,id);
        require(chosen[id].length==t.redundancy,"insufficient accepting hosts");
        emit TaskAsset(id,token,t.fee);
    }
    function withdrawTokenRoyalty(bytes32 mepId,address token) external guard returns(uint256 amt){
        (address ben,)=meps.termsOf(mepId);require(msg.sender==ben,"beneficiary");
        amt=tokenRoyalties[mepId][token];require(amt>0,"nothing");tokenRoyalties[mepId][token]=0;TokenTransfer.send(token,ben,amt);
    }
    function _assetPay(bytes32 id,address to,uint256 amount) internal {
        address token=paymentToken[id];if(token!=address(0)){TokenTransfer.send(token,to,amount);return;}
        (bool ok,)=to.call{value:amount}("");require(ok,"pay/refund");
    }
    ...
    function postTask(Task calldata t, bytes32 nonce) external payable guard returns (bytes32 taskId) { taskId = PorwMeshHash.taskId(t, nonce); _post(t, taskId); }
    ...
    function postBatch(Task calldata t, uint32 runs, bytes32 nonce) external payable guard returns (bytes32 taskId) {
    ...
    function _post(Task calldata t, bytes32 taskId) internal {
        require(msg.value == (paymentToken[taskId]==address(0)?t.fee:0), "fee");
    ...
    function _draw(bytes32 taskId, bytes32 mepId, uint64 epoch, uint8 redundancy) internal {
        ...
        for (uint32 j = 0; out.length < redundancy && j < 64 * uint32(redundancy); j++) {
            ...
            if(paymentToken[taskId]!=address(0)&&!acceptedToken[cand][paymentToken[taskId]])continue;
    ...
    function settle(bytes32 taskId) external guard {
        StoredTask storage st = tasks[taskId];
        ...
        for (uint256 i = 0; i < ex.length; i++) {
            ...
            for (uint256 j = i + 1; j < ex.length; j++) {
                ...
                if (results[taskId][ex[i]].execRoot != results[taskId][ex[j]].execRoot) { // the ROOT: see _same
                    disputeResolved[taskId]=false; st.disputed = true; emit DisputeOpened(taskId, ex[i], ex[j]);
                    IAssetDisputeOpener(disputes).openDispute(taskId, ex[i], ex[j]);
                    return;
                }
            }
        }
    ...
    function challengeResult(bytes32 taskId, Result calldata r) external payable {
        ...
        disputeResolved[taskId]=false; st.disputed = true;
        emit ResultChallenged(taskId, msg.sender, r.execDigest);
        emit DisputeOpened(taskId, ref, msg.sender);
        IAssetDisputeOpener(disputes).openDispute(taskId, ref, msg.sender);
    }
    ...
    function onDisputeResolved(bytes32 taskId, address loser, address winner) external {
        ...
        StoredTask storage st = tasks[taskId];
        require(st.exists && st.disputed && !disputeResolved[taskId], "task");
        disputeResolved[taskId]=true;
    ...
    function _pay(bytes32 taskId, address[] memory ex, address ref) internal {
        ...
        if (agree == 0) { _assetPay(taskId,st.client,st.t.fee); emit TaskSettled(taskId, bytes32(0), new address[](0)); return; }
        ...
        if (cut > 0) {
            address token=paymentToken[taskId];
            if(token==address(0)){royalties[st.t.mepId]+=cut;emit RoyaltyAccrued(taskId,st.t.mepId,cut);}
            else {
                if(tokenRoyaltyCallback[taskId]){TokenTransfer.send(token,ben,cut);ITokenRoyaltyReceiver(ben).onTokenRoyalty(st.t.mepId,token,cut);}
                else tokenRoyalties[st.t.mepId][token]+=cut;
                emit TokenRoyaltyAccrued(taskId,st.t.mepId,token,cut);
            }
        }
        ...
        for (uint256 i = 0; i < ex.length; i++) if (submitted[taskId][ex[i]] && _same(taskId, ex[i], ref)) { paid[p++] = ex[i]; _assetPay(taskId,ex[i],share); }
        if(paymentToken[taskId]!=address(0))_assetPay(taskId,st.client,(st.t.fee-cut)%agree);
