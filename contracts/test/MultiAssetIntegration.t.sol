...
import {MultiAssetTaskMarket as TaskMarket} from "../src/MultiAssetTaskMarket.sol";
import {TaskMarket as LegacyMarket} from "aigg-porw/mesh/TaskMarket.sol";
import {TestToken} from "./MultiAssetTaskMarket.t.sol";
import {ExecutionDisputes} from "aigg-porw/mesh/ExecutionDisputes.sol";
import { MeshFixtures as FX } from "../lib/aigg-porw/contracts/evm/test/fixtures/MeshFixtures.sol";
...
contract MultiAssetIntegrationTest is Test {
    ...
    function setUp() public {
        ...
        market = new TaskMarket(meps, inst, cm, uint64(50)); disp = new ExecutionDisputes(meps, inst, LegacyMarket(address(market)), 10, SLASH);
    ...
    function tokenSetup() internal returns(TestToken token){
        uint256[] memory w=new uint256[](2);w[0]=1;w[1]=1;enrol(w,all(2,true));
        token=new TestToken();token.mint(address(this),1 ether);token.approve(address(market),1 ether);market.setTokenAllowed(address(token),true);
        for(uint i;i<who.length;i++){vm.prank(who[i]);market.setAcceptedToken(address(token),true);}
    }
    function test_tokenDisagreementResolvesAndPaysCorrectAsset() public {
        TestToken token=tokenSetup();ITaskMarket.Task memory t=task(2);
        bytes32 id=market.postTokenTask(t,address(token),"tokens");address[] memory ex=market.executors(id);
        ...
        assertFalse(market.disputeResolved(id));vm.prank(ex[0]);disp.revealRoots(id,FX.actRootsA());vm.roll(block.number+11);disp.timeout(id);
        assertTrue(market.disputeResolved(id));assertEq(token.balanceOf(ex[0]),t.fee);assertEq(token.balanceOf(ex[1]),0);
    }
    function test_tokenSuccessfulChallengeIsTerminalAndDoesNotClawBackFee() public {
        TestToken token=tokenSetup();bytes32 id=market.postTokenTask(task(2),address(token),"challenge");address[] memory ex=market.executors(id);
        (ITaskMarket.Result memory rb,,)=FX.resultB0();(ITaskMarket.Result memory ra,,)=FX.resultA0();
        submit(id,ex[0],rb.execDigest,rb.execRoot);submit(id,ex[1],rb.execDigest,rb.execRoot);market.settle(id);
        market.setTokenAllowed(address(token),false); // revocation affects new posts, not existing settlement/challenges
        vm.deal(CHAL,1 ether);vm.prank(CHAL);market.challengeResult{value:0.05 ether}(id,ra);
        assertFalse(market.disputeResolved(id));vm.prank(CHAL);disp.revealRoots(id,FX.actRootsA());vm.roll(block.number+11);disp.timeout(id);
        assertTrue(market.disputeResolved(id));(,,,,,,,,bool repudiated)=market.tasks(id);assertTrue(repudiated);
        assertEq(token.balanceOf(ex[0])+token.balanceOf(ex[1]),task(2).fee);assertEq(CHAL.balance,1 ether+SLASH);
    }
}
