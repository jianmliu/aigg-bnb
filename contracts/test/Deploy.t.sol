...
contract DeployTest is Test {
    ...
    function test_deploy_the_genesis_collection_and_list_it() public {
        ...
        vm.setEnv("OWNER", vm.toString(DEFAULT_SENDER)); vm.setEnv("TREASURY_OWNER", vm.toString(DEFAULT_SENDER)); vm.setEnv("TREASURY_DESTINATION", vm.toString(address(0x5AFE))); // and so are the owners
        ...
        assertEq(c.BASE_SHARE_BPS(), 1000); assertEq(c.SALE_ROYALTY_BPS(), 500); assertEq(c.owner(), DEFAULT_SENDER);
        // no TREASURY named: the collection's immutable treasury is a router whose destination can change, and the base's share goes there too
        TreasuryRouter tr = TreasuryRouter(payable(c.TREASURY())); assertTrue(address(tr).code.length > 0, "deployed"); assertEq(tr.owner(), DEFAULT_SENDER); assertEq(tr.destination(), address(0x5AFE)); assertEq(c.BASE_VENDOR(), address(tr));
