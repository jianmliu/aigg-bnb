...
| `gateway/` | the OpenAI-compatible adapter (`docs/GATEWAY.md`, milestone 0): `POST /v1/responses` becomes an on-chain task paid by the gateway's own wallet, and the answer is a receipt. `npm run gateway`; it is not part of the relayer, because it holds money |
