...
| `docs/SHARED_VRF_ROUNDS.md` | shared task-assignment beacon: one VRF request per bounded round, host capacity commitments, recovery and rollout limits |
...
- **The collection, again, with the mainnet revision** (same day, from `main` at `8bfb263`; the contract's source is
  ...
  (`0xE0a5…7AA4`) is immutable and predates all of this; nobody adopted from it, and the curator took it off the whitelist the same day
  (tx `0x76723efb…c344`, with the reason on-chain: superseded by the collection above). Cost: 0.00055 tBNB.
