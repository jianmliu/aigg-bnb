...
# HATCH_BOUNTY (wei) and ROYALTY_BPS for a network whose UNIT is smaller. TREASURY defaults to the deployer and is
...
echo "$OUT" | grep -E "^  (collection|genesis size|listed|0x)" || true
