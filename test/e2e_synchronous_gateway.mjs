...
const VRF=process.argv.includes('--vrf'),MODE=VRF?'synchronous-vrf-v1':'synchronous-v1',ADMISSION=1000n;
...
try {
 ...
 dep.addresses.market=await H.create(owner,VRF?'VrfSynchronousTaskMarket':'SynchronousTaskMarket',[dep.addresses.meps,dep.addresses.instances,dep.addresses.claims,120n,40n,300n,...(VRF?[{coordinator,keyHash:'0x'+'11'.repeat(32),subId:1n,requestConfirmations:3,callbackGasLimit:200000,nativePayment:true,waitBlocks:500n,activationBlocks:200n,readyTTL:10000n,feeRecipient:owner.account.address},['0x'+'00'.repeat(20)],[ADMISSION]]:[])]);
 if(VRF)admission=await H.readFrom(owner,dep.addresses.market,'VrfSynchronousTaskMarket','admission');
 const fulfill=async id=>{const info=await H.readFrom(owner,admission,'VrfAdmission','requestInfo',[id]);const abi=parseAbi(['function fulfill(address,uint256,uint256)']);await owner.pub.waitForTransactionReceipt({hash:await owner.wallet.writeContract({address:coordinator,abi,functionName:'fulfill',args:[admission,info[0],42n]})});};
 ...
 const gatewayEnv={GATEWAY_BEARER:'test',GATEWAY_VRF_ADMISSION_BUDGET_WEI:'1000000',GATEWAY_MODELS:`sync=${mepId}`,GATEWAY_STATE:path.join(tmp,'state.json'),GATEWAY_POLL_MS:'100',GATEWAY_RESULT_TIMEOUT_MS:'15000'};let G=await H.startGateway(R,H.KEYS[4],gatewayEnv);cleanup.push(()=>G.stop());
 ...
 let delivered=false;const agreement=call({model:'sync',seed:7,max_output_tokens:4,...(VRF?{background:true}:{})}).then(async r=>{
  if(VRF){const id=r.body.id;for(;;){try{const response=await fetch(G.url+'/v1/responses/'+id,{headers:{authorization:'Bearer test'}});const body=await response.json();if(['completed','failed'].includes(body.status)){r={status:response.status,body};break;}}catch{}await H.sleep(100);}}
 ...
 if(VRF){assert.equal(Number((await gateway.market.read.sessionState([agreedId]))[0]),6);assert.equal((await gateway.market.read.executors([agreedId])).length,0);
  ...
  assert.equal((await H.readFrom(owner,admission,'VrfAdmission','requestInfo',[agreedId]))[0],1n,'restart keeps original VRF request');await anvil.mine(64);await fulfill(agreedId);await anvil.mine(64);await wait(async()=>Number((await gateway.market.read.sessionState([agreedId]))[0])===1);}
 await anvil.mine(64);
 ...
 if(VRF){
  ...
  const info=await H.readFrom(owner,admission,'VrfAdmission','requestInfo',[taskId]);await to(Number(info[1])+1);
 }else{
