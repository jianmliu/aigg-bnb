...
try {
 ...
 dep.addresses.market=await H.create(D,'SynchronousTaskMarket',[dep.addresses.meps,dep.addresses.instances,dep.addresses.claims,1000n,1000n,24000n]);
 ...
 await anvil.mine(64);
 R=await H.startRelayer(dep,H.KEYS[3],[baseId],{env:{PORW_VERIFICATION_MODE:'synchronous-v1',PORW_SYNC_CONFIRMATIONS:'0',PORW_SPONSOR_EPOCH_GAS:'160000000',PORW_SPONSOR_DAY_GAS:'350000000'}});cleanup.push(()=>R.stop());
 ...
 for(const walletIndex of [1,2]){const W=H.clientsFor(dep,H.KEYS[walletIndex]);assert.equal(await W.instances.read.isEligible([W.account.address,childId,BigInt(epoch+1)]),true);}
 ...
 await D.pub.waitForTransactionReceipt({hash:await D.market.write[BATCH?'postBatch':'postTask'](BATCH?[task,runs.length,nonce]:[task,nonce],{value:task.fee})});
 const taskId=await D.market.read.taskId([task,'0x'+'00'.repeat(20),nonce,BATCH?runs.length:0,D.account.address]);assert.equal((await D.market.read.executors([taskId])).length,2);await anvil.mine(64);
 ...
 const sessions=await Promise.all(pages.map(page=>page.evaluate(()=>window.app.state.delegation.session)));
 if(RESTART){
 ...
 assert.equal(await D.market.read.settledDigest([taskId]),H.hex(expected.result.execDigest));assert.equal(baseHits,2);assert.equal(deltaHits,2);
 if(!RESTART&&!DISAGREE&&!BATCH){
  for(const page of pages)await page.evaluate(()=>window.appActions.armSynchronousSession());await anvil.mine(64);
  const nextNonce='0x'+'72'.repeat(32);await D.pub.waitForTransactionReceipt({hash:await D.market.write.postTask([task,nextNonce],{value:task.fee})});
