...
export async function startRelayer(dep, key, mepIds, { relayPort = 0, apiPort = 0 } = {}) {
  const cfgPath = path.join(root, "deployments/relayer-test.json"); fs.writeFileSync(cfgPath, JSON.stringify({ deployment: "deployments/31337.json", rpc: dep.rpc, privateKey: key, meps: mepIds, relayPort, apiPort, pollMs: 500, name: "test-relayer" }));
  const child = fork(path.join(root, "relayer/relayer.mjs"), ["--config", cfgPath], { stdio: ["ignore", "pipe", "pipe", "ipc"] }); let log = ""; child.stdout.on("data", (d) => (log += d)); child.stderr.on("data", (d) => (log += d));
