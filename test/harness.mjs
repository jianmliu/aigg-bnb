...
/** `baseMepFemale`: the base brain the collection answers for; `royaltyBps` > 0 registers its flies under terms and needs the market */
export const deployCollection = (dep, bounty, key = KEYS[0], { baseMepFemale = ZERO_WORD, royaltyBps = 0 } = {}) => create(clientsFor(dep, key), "FlyCollection", [keccak256("0x0f"), keccak256("0x0e"), GENESIS.root, 2, FLY_PRICE, 0n, FLY_BREED_FEE, bounty, FLY_TREASURY, dep.addresses.meps, ZERO_ADDR, ZERO_ADDR, baseMepFemale, ZERO_WORD, royaltyBps ? dep.addresses.market : ZERO_ADDR, royaltyBps]);
