...
//   - each brain is priced on its own row, at its own wei_per_step x USD/BNB x margin (a heavier brain costs a host more)
...
// ---- the gateway ----
const models = [{ id: "flywire-783-min5", aliases: ["mep:0x31"], exec: "int-lif", neurons: 139255, providers: 3, available: true, min_redundancy: 2, wei_per_step: "100000000000", set_factors: { ocelli: 0.28, pheromone: 2.64 } },
  { id: "flywire-783-min2", aliases: ["mep:0x79"], exec: "int-lif", neurons: 139255, providers: 0, available: false, min_redundancy: 2, wei_per_step: "154000000000" }]; // 1.54x: the heavier export costs a host more
...
{ const r = await run("apply", online); const [g] = db.groups, [a] = db.accounts, [c] = db.channels;
  ...
  const byModel = Object.fromEntries(c.model_pricing.map((r) => [r.models.slice().sort().join(","), r]));
  const r5 = byModel["flywire-783-min5,mep:0x31"], r2 = byModel["flywire-783-min2,mep:0x79"];
  check(`one pricing row per brain, each at its own price: min5 ${PRICE}, min2 ${(PRICE * 1.54).toPrecision(6)} USD per token`, c.model_pricing.length === 2 && !!r5 && !!r2
    && Math.abs(r5.output_price - PRICE) < 1e-12 && Math.abs(r2.output_price - PRICE * 1.54) < 1e-9 && r5.platform === "openai" && r2.platform === "openai");
  check("and a token of either kind costs the same on each row: an input token is the call's gas, in steps of that brain", r5.input_price === r5.output_price && r2.input_price === r2.output_price && r5.billing_mode === "token");
...
{ models.push({ id: "fly-17", aliases: ["mep:0xf1"], exec: "int-lif", neurons: 139255, providers: 1, available: false, min_redundancy: 2, wei_per_step: "100000000000" }); const n = writes.length;
  ...
  const row17 = c.model_pricing.find((x) => x.models.includes("fly-17"));
  check("apply updates the account -- credentials sent WHOLE, because ai.gg replaces them -- and the channel: a row for the new fly, at its own price, and every row at the new BNB rate",
    r.code === 0 && writes.length === n + 2 && writes.at(-2).method === "PUT" && a.credentials.api_key === BEARER && a.credentials.base_url === "https://gateway.example" && "fly-17" in a.credentials.model_mapping
    && c.model_pricing.length === 3 && !!row17 && Math.abs(row17.output_price - 1e-7 * 700 * 1.25) < 1e-12 && row17.input_price === row17.output_price && a.extra.openai_passthrough === true); }
