...
{ const r = await run("apply", online); const [g] = db.groups, [a] = db.accounts, [c] = db.channels;
  ...
  check(`the pricing row is platform "openai", token mode, ${PRICE} USD per token of EITHER kind (input tokens are the call's gas)`, c.model_pricing.length === 1 && row.platform === "openai" && row.billing_mode === "token" && Math.abs(row.output_price - PRICE) < 1e-12 && row.input_price === row.output_price && row.models.length === 4);
...
{ models.push({ id: "fly-17", aliases: ["mep:0xf1"], exec: "int-lif", neurons: 139255, providers: 1, available: false, min_redundancy: 2, wei_per_step: "100000000000" }); const n = writes.length;
  ...
  check("apply updates the account -- credentials sent WHOLE, because ai.gg replaces them -- and the channel, at the new price", r.code === 0 && writes.length === n + 2 && writes.at(-2).method === "PUT" && a.credentials.api_key === BEARER && a.credentials.base_url === "https://gateway.example" && "fly-17" in a.credentials.model_mapping && c.model_pricing[0].models.includes("fly-17") && Math.abs(c.model_pricing[0].output_price - 1e-7 * 700 * 1.25) < 1e-12 && c.model_pricing[0].input_price === c.model_pricing[0].output_price && a.extra.openai_passthrough === true); }
