...
//   - ONE price for every model: a heavier brain costs more TOKENS, not a higher rate, so pricing it higher here too
//     would charge its factor twice
...
// ---- the gateway ----
const models = [{ id: "flywire-783-min5", aliases: ["mep:0x31"], exec: "int-lif", neurons: 139255, providers: 3, available: true, min_redundancy: 2, wei_per_token: "100000000000", tokens_per_step: 1, set_factors: { ocelli: 0.28, pheromone: 2.64 } },
  { id: "flywire-783-min2", aliases: ["mep:0x79"], exec: "int-lif", neurons: 139255, providers: 0, available: false, min_redundancy: 2, wei_per_token: "100000000000", tokens_per_step: 1.54 }]; // the heavier export: more tokens a step, same price a token
...
{ const r = await run("apply", online); const [g] = db.groups, [a] = db.accounts, [c] = db.channels;
  ...
  check(`one price for every model (${PRICE} USD a token), because a heavier brain costs more tokens and not a higher rate`,
    c.model_pricing.length === 1 && row.platform === "openai" && row.billing_mode === "token" && Math.abs(row.output_price - PRICE) < 1e-12 && row.models.length === 4);
  check("and a token of either kind costs the same: an input token is the call's gas in that same unit", row.input_price === row.output_price);
...
{ models.push({ id: "fly-17", aliases: ["mep:0xf1"], exec: "int-lif", neurons: 139255, providers: 1, available: false, min_redundancy: 2, wei_per_token: "100000000000", tokens_per_step: 1 }); const n = writes.length;
  ...
  check("apply updates the account -- credentials sent WHOLE, because ai.gg replaces them -- and the channel: the new fly on the row, at the new BNB rate",
    ...
    && c.model_pricing.length === 1 && c.model_pricing[0].models.includes("fly-17") && Math.abs(c.model_pricing[0].output_price - 1e-7 * 700 * 1.25) < 1e-12 && c.model_pricing[0].input_price === c.model_pricing[0].output_price && a.extra.openai_passthrough === true); }
