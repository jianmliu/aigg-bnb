...
async function tx(label, fn) {
  try { const hash = await fn(); const r = await ch.pub.waitForTransactionReceipt({ hash }); status.txs.push({ label, hash, ok: r.status === "success", gas: Number(r.gasUsed) }); log(label, hash.slice(0, 12), r.status, Number(r.gasUsed), "gas"); return { hash, ok: r.status === "success", gasUsed: Number(r.gasUsed) }; }
  catch (e) { const msg = String(e.shortMessage || e.message).slice(0, 200); status.errors.push({ label, msg }); log(label, "FAILED", msg); return { ok: false, error: msg }; }
}
...
async function tick() {
  ...
  if (beaconOn) {
    ...
    if (bn + BigInt(bcfg.commit) >= nextStart && bn < nextStart && !secrets.has(e + 1)) {
      ...
      const r = await tx(`beacon.commit(${e + 1})`, () => ch.beacon.write.commit([hex(h)], { value: bcfg.deposit })); if (r.ok) { secrets.set(e + 1, secret); status.commits.push(e + 1); }
    }
    if (secrets.has(e) && bn >= start && bn < start + BigInt(bcfg.reveal)) {
      const c = await ch.beacon.read.commits([BigInt(e), ch.account.address]);
      if (!c[1]) { const r = await tx(`beacon.reveal(${e})`, () => ch.beacon.write.reveal([BigInt(e), secrets.get(e)])); if (r.ok) status.reveals.push(e); }
    }
  }
  ...
  if (rolled === "0x" + "0".repeat(64)) {
    ...
    if (ready) { const r = await tx(`claims.rollEpoch(${e})`, () => ch.claims.write.rollEpoch()); if (r.ok) status.epochsRolled.push(e); }
  ...
  for (const [id, M] of meps) {
    ...
    if (prev >= 0 && M.aggregators.has(prev) && !M.posted.has(prev)) {
      ...
      if (already[0] === "0x" + "0".repeat(64)) { const r = await tx(`claims.postEpochRoot(${id.slice(0, 10)}, ${prev}, ${call.count} claims)`, () => ch.claims.write.postEpochRoot([id, BigInt(prev), call.root, BigInt(call.count)])); if (r.ok) { M.posted.add(prev); status.rootsPosted.push({ mep: id, epoch: prev, root: call.root, count: call.count }); } }
...
const api = http.createServer(async (req, res) => {
  try {
    ...
    if (u.pathname === "/tx/delegate") { if (!(await isBonded(b.instance))) return json(res, 403, { error: "instance not bonded" });
      return json(res, 200, await tx(`instances.delegateBySig(${b.instance.slice(0, 10)})`, () => ch.instances.write.delegateBySig([b.instance, b.session, BigInt(b.expiry), b.sig]))); }
    if (u.pathname === "/tx/materialize") { const id = b.mep.toLowerCase(); const M = meps.get(id); const A = M && M.aggregators.get(Number(b.epoch)); const p = A && A.proofFor(b.instance);
      ...
      const l = p.payload.leaf; return json(res, 200, await tx(`claims.materializeClaim(${b.instance.slice(0, 10)}, ${b.epoch})`, () => ch.claims.write.materializeClaim([id, BigInt(b.epoch), ch.account.address, BigInt(p.payload.index), { instance: l.instance, partialsRoot: l.partialsRoot, coverageBytes: BigInt(l.coverageBytes), deviceId: l.deviceId, execDigest: l.execDigest, stimulusSeed: l.stimulusSeed, signature: l.signature }, p.payload.proof]))); }
    if (u.pathname === "/tx/result") { const signer = await ch.instances.read.resolve([b.signer]); if (signer === "0x0000000000000000000000000000000000000000") return json(res, 403, { error: "signer not bonded/delegated" });
      return json(res, 200, await tx(`market.submitResult(${b.taskId.slice(0, 10)})`, () => ch.market.write.submitResult([b.taskId, { execDigest: b.execDigest, execRoot: b.execRoot }, b.signature]))); }
    if (u.pathname === "/tx/settle") return json(res, 200, await tx(`market.settle(${b.taskId.slice(0, 10)})`, () => ch.market.write.settle([b.taskId])));
