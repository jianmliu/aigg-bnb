...
export function relayerFromEnv() {
  const e = process.env; return { privateKey: e.PORW_RELAYER_KEY || null, meps: e.PORW_MEP_IDS ? e.PORW_MEP_IDS.split(",").map((s) => s.trim()).filter(Boolean) : null,
    ...
    relayPort: e.PORW_RELAY_PORT ? Number(e.PORW_RELAY_PORT) : null, apiPort: e.PORW_API_PORT ? Number(e.PORW_API_PORT) : (e.PORT ? Number(e.PORT) : null), // PORT: what a PaaS hands us host: e.PORW_HOST || null, pollMs: e.PORW_POLL_MS ? Number(e.PORW_POLL_MS) : null, name: e.PORW_RELAYER_NAME || null,
