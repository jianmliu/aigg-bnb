...
export function clients(dep, privateKey = null) {
  ...
  return { chain, pub, wallet, account, instances: c(a.instances, InstanceRegistryAbi), claims: c(a.claims, ClaimManagerAbi), market: c(a.market, TaskMarketAbi), meps: c(a.meps, MEPRegistryAbi), beacon: a.beacon ? c(a.beacon, BeaconAbi) : null, relays: c(a.relays, RelayRegistryAbi), collection: a.collection ? c(a.collection, FlyCollectionAbi) : null, whitelist: a.whitelist ? c(a.whitelist, CollectionWhitelistAbi) : null };
}
