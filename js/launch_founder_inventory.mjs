...
export async function launchInventory({c,cfg,journal,activate=true,smoke=true}) {
 ...
 assert(cfg.protocol===undefined||['synchronous-v1','synchronous-vrf-v1','synchronous-vrf-rounds-v1'].includes(cfg.protocol),'unknown protocol');
 ...
 if(vrf)assert.equal(await read(cfg.market,'VrfSynchronousTaskMarket','admissionVersion'),rounds?3n:2n);
...
if(process.argv[1]&&path.resolve(process.argv[1])===fileURLToPath(import.meta.url)) {
 ...
 if(process.argv.includes('--vrf')||process.argv.includes('--vrf-rounds')){assert(baseEnrollment,'VRF inventory requires --base-enrollment');assert(journal!==legacyJournal,'VRF inventory requires a separate journal');cfg.protocol=process.argv.includes('--vrf-rounds')?'synchronous-vrf-rounds-v1':'synchronous-vrf-v1';}
