...
// (aigg-porw model_id / synapseRoot / exec kind -- all of them derivable from the bytes) plus its
// Greenfield pointer. Step count and commit stride are NOT registry fields: they ride on each task.
...
const m = { modelId: f.modelId, schemeDigest: f.schemeDigest, execKind: f.execKind, neurons: f.neurons, synapses: f.synapses, synapseRoot: f.synapseRoot, weightsDA: stringToHex(pointer) };
...
const on = await c.meps.read.getMEP([f.mepId]); console.log(`on-chain MEP ${f.mepId}: modelId ${on.modelId} neurons ${on.neurons} synapses ${on.synapses} synapseRoot ${on.synapseRoot} weightsDA ${Buffer.from(on.weightsDA.slice(2), "hex").toString()}`);
