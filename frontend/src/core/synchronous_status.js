...
export function sessionStatus(s) {
  ...
  const pending=!!s.pendingTasks?.length || !!s.pendingTask && !/^0x0{64}$/i.test(s.pendingTask);
  ...
  const detail=s.error ? `State unavailable: ${s.error}. Closure has not been confirmed.`
    : s.phase==='collecting round'?'Your declared slots reserve tasks in a bounded round. Keep this page open through round sealing, randomness and every assigned task’s verification.': s.phase==='awaiting randomness'?'Your capacity is reserved in a locked candidate pool. Keep this page open while VRF selects two executors. Unselected hosts are released after confirmed allocation.': s.phase==='inconclusive'?'This result was not accepted as a completed experiment. Check the task refund status.'
