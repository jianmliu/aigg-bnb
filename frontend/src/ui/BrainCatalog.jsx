...
export function BrainCatalog({ visible, listing = false, steps }) {
  ...
  const familyMode = !listing && !!s.deployment?.familyHosting;
  const models = listing ? s.meps : C.hostingModels();
  const matches = models.filter(m => (scope !== 'selected' || s.hosted.has(m.mepId)) &&
  ...
  return <div id={`${prefix}Catalog`}>
    <div className="catalog-controls">
      <label htmlFor={`${prefix}Search`}>{familyMode ? 'Find a model family' : 'Find a brain'}
      ...
      {!listing && <label htmlFor="hostScope">Show
        <select id="hostScope" value={scope} onChange={e => { setScope(e.target.value); setPage(1); }}>
          <option value="all">{familyMode ? 'All available model families' : 'All available brains'}</option><option value="selected">Selected for hosting ({s.hosted.size})</option>
        </select>
      </label>}
    </div>
    ...
    <div className={listing ? 'listings' : 'brains'}>
      {shown.map((m, i) => <BrainCard key={m.mepId} listing={listing} index={i} mep={m}
        ...
        onSelect={() => listing ? C.setActive(m.mepId) : C.selectHostFamily(m.mepId)} onHost={v => C.host(m.mepId, v)} />)}
    </div>
    {matches.length > 12 && <div className="row tight center flies-paging">
      ...
      <span>Page {current} of {pages} · {matches.length} {familyMode ? 'model families' : 'brains'}</span>
