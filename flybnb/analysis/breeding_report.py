...
0.875. The slope actually measured is the narrow-sense heritability h^2 of the phenotype under this cross; how far it falls
below 0.875 is how much of the phenotype lives in interactions between connections, which recombination breaks up.
...
out = {"individuals": {"founders": len(founders), "random": len(rand), "high": len(high), "low": len(low)}, "additive_ceiling": 0.875, "phenotypes": {}}
...
L = [f"# Breeding pilot: {len(founders)} founders, {len(rand)} randomly mated offspring, {len(high)} + {len(low)} offspring of the divergent selection lines\n",
     f"Additive ceiling under this cross (one connection in eight is redrawn): 0.875. {out['h2_distribution']['n']} phenotypes; h² median {out['h2_distribution']['median']:.2f} (quartiles {out['h2_distribution']['q25']:.2f} to {out['h2_distribution']['q75']:.2f}); {100 * out['h2_distribution']['above_0.5']:.0f}% above 0.5.\n" if len(hs) else ""]
