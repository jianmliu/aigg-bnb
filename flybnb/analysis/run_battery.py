...
Recipes are the collection's format (min_syn 5, mean ratio 0.92, record-level inheritance, mutation rate 1/8), so an
individual here is exactly the brain its recipe would register. One output line per individual: its recipe and delta id, its
...
def _init(base_path, mid, battery, spec):
    try:
        ...
        _S["dn"] = np.array(battery["readout"]["neuron_index"], dtype=np.int64)
...
def recipe(iid):
    ...
    x = _S["spec"][iid]; B = _S["B"]; name = FD.fit_name(x.get("name", iid), _S["nl"])
    if x["kind"] == "base": d = None
    elif x["kind"] == "founder": d = FD.encode_delta3(_S["mid"], B["n"], name, "", FD.ZERO_ID, FD.ZERO_ID, x["seed"], granularity=0, min_syn=5, mut_rate_q32=FD.MUT_ALWAYS, mean_ratio_q16=MEAN_RATIO_Q16, layout=1)
    else: d = FD.encode_delta3(_S["mid"], B["n"], name, "", FD.delta_id(recipe(x["a"])), FD.delta_id(recipe(x["b"])), x["seed"], granularity=0, min_syn=5, mut_rate_q32=MUT_Q32, mean_ratio_q16=MEAN_RATIO_Q16, layout=1)
    _S["recipes"][iid] = d; return d

def _job(iid):
    t0 = time.time(); x = _S["spec"][iid]; B = _S["B"]; c = _S["c"]; d = recipe(iid)
    if d is None: g = np.where(c >= 5, c, 0)
    else:
    ...
    for st in _S["bat"]["stimuli"]:
        for seed in _S["bat"]["seeds"]:
            count, late = IL.run(net, seed, _S["bat"]["steps"], st["neuron_index"]); l = late[dn]; nz = np.flatnonzero(l)
...
def main():
    ...
    assert "0x" + mid.hex() == BASE_MODEL_ID, f"not the published base: 0x{mid.hex()}"
