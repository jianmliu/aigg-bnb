...
All three runs below were made on the **pre-migration** testnet mesh (MEPRegistry `0xC779151b…`), which the family and VRF meshes of #93/#97 replaced (MEPRegistry `0x950fab76…`). What they establish is about the brains — their bytes, `model_id`s and counts — and that did not move; where an id did move, it is said below.
...
**And on the published wiring** (task `0x553db67d…c6c6`, settled on `0xcdada85b…5c4d`): all 42 digests equal the dataset's `base` row — the row `run_battery.py` writes, not a reference built for the purpose. It is the evidence that the dataset's own published rows are what the network produces. Its MEP is a root, and a root's id is its content address, so `0x93496ed953a2…` is unchanged by the migration and still pinned. This is the run the substrate could not be, and the pair is now the evidence for the distinction below — the same battery, the same weight unit, two male payloads, and **not one of the 42 digests is shared between them**. `test/flybnb_battery.mjs` checks both directions.

**And then on a minted fly** (task `0x4df627a8…7d32`, settled on `0xc8ccb0f9…5ec6`). `fly #101` of the pre-migration founder collection is, by delta id, the pilot founder `M000` of `flybnb/results/male/pilot/`: the same FLYDELTAv3 recipe, seed 1000, the male dispersion table and mean ratio. Its battery was posted as one task, executed, settled — and **all 42 counts digests equal the row already committed in this repository**, checked against those rows themselves rather than a summary. That token and its MEP (`0xd46638da…`) belong to the paused inventory. The brain does not: the same recipe is founder index 100 of the replacement inventory (`flybnb/genesis/founder-profiles-v2.json`), with the same `model_id` `0x8af16bb9…` and the same synapse root — `test/flybnb_battery.mjs` checks that too. Only the wrapping moved: a derived id now binds its root base as well as its terms (aigg-porw #37), so the live MEP id is a different number for the same bytes.

That run needed no special reference, which is the individual case being clean: a genotype zeroes what falls under `min_syn`, so a fly's payload and its offline row are the same network. Two things it did need, both of which a base payload does not: the payload is the substrate with the fly's delta applied (`FD.apply_any`), and the MEP id **wraps the collection's royalty terms** (1000 bps), so a node must be told them or the same bytes reproduce a different id — `loadModel({ terms })`, exactly as the relayer did before base enrolment. On a base-enrolment mesh a derived id also binds the root base, which `test/live_male_battery.mjs` does not reconstruct; it refuses such a brain rather than guess, and for minted flies `battery/worker.mjs` is the path.

**The reference is not the dataset's `base` row, and the difference is the point.** `run_battery.py` calls the *published wiring* the base: the ≥ 2-synapse export thresholded at the population's `min_syn` (five). What `registerMEP` points at is the ≥ 2 export itself — the substrate individuals are drawn on — which keeps 15,283,237 records against 6,242,118 and is a denser network with different dynamics. A node holds the payload as it is. Joining the run against the dataset's base row makes all 42 digests differ, which is the right answer to the wrong question; it is how this was found. Individuals do not have the problem: a FLYDELTA genotype zeroes what falls under `min_syn`, so an individual's payload and its offline row are the same network. If a task should reproduce the dataset's base row, what belongs on chain is the ≥ 5 export as its own MEP — which is why `malecns-v1.0-min5` is registered beside the substrate (#81), as the female pair already was.

**What these runs are that nothing else in the repository is: a second implementation.** Every production check executes the same WASM kernel — the two hosts of a synchronous task, and the "independent battery replay" in `battery/worker.mjs`, which calls `node.execute` on the same `PorwNode`. That catches a host that lies. It cannot catch a bug every copy of the kernel shares, because every copy agrees on the wrong answer; the verification design says as much (*extra wallets do not establish … software diversity*). These three runs join the network's digests with `flybnb/analysis/intlif.py`, written separately in numpy. Rows that enter the dataset through the production pipeline got no such check; `battery/audit.mjs` (below) now audits a sample of them.

## Auditing the production pipeline (`battery/audit.mjs`)

The runs above were one-off. `battery/audit.mjs` makes the same check a routine: it takes the artifacts `battery/worker.mjs` archives (`<BATTERY_STATE>/artifacts/*.json`), draws a sample, and for every sampled result

1. **rebuilds the executed payload** from content addresses: the root base (found by `model_id` among `--base` files, or with `--fetch` on the Pages mirror the committed profiles name) with the fly's FLYDELTA recipe applied in place (`FD.apply_any`; found by delta id in `--recipes`, or with `--fetch` on the founder-asset mirror), and requires the result's `model_id` to be the archived `modelId`. A root MEP is its base as it is. The battery file must hash to the archive's `batteryVersion`, and a derived fly must be drawn on the battery's population base, whose weight unit it is;
2. **runs the battery with `intlif.py`** at the population's weight unit over the payload **as registered** — every record, no `min_syn` threshold (the section above explains why a root substrate is not the dataset's `base` row);
3. **joins the counts digests run by run** (`checkAgainstOffline`, plus the checks a join over the archive's own rows would miss: a run missing, a run twice, a run out of the battery's order). Every run that differs is named with both digests; nothing is averaged.

```bash
node battery/audit.mjs --battery flybnb/battery/battery-male-v1.json --archives <BATTERY_STATE>/artifacts \
    --base malecns-v1.0-min2.bin --cache ~/.cache/flybnb-audit [--fetch] [--rate 0.1 | --all] [--salt S] [--workers 6] --out audit.json
```

It exits non-zero if any sampled result disagrees **or cannot be rebuilt** — a missing recipe, a wrong `model_id` or another battery version is a failure, not a skip. It reads no chain and writes nothing but its report; `--fetch` is the only network access, and fetched bytes are cached only after their content address checks.

**What it costs, and the default rate.** One male battery (42 runs) of fly #101 = `M000`, rebuilt from the MaleCNS min2 base and its recipe: 8 s to rebuild and hash, 60 s for the battery on one core (Apple silicon, 1.7 GB resident; the pilot's machine took about 2 min per individual), 39 s wall with `--workers 6`. The default rate is **0.1**: on average 7–12 CPU-seconds per delivered battery. A bug that every copy of the kernel shares and that touches every battery is then caught with probability 1 − 0.9^N after N delivered results — 65% by the 10th, 95% by the 29th, 99% by the 44th; one that touches a fraction f of batteries takes about 10/f results. While the dataset is small, `--all` is cheap enough.

**The draw** is by hash: a result is audited iff keccak256(salt ‖ taskId) < rate · 2^256. A rerun draws the same results, a result's fate does not depend on which others are in the directory, and another salt draws an independent sample. Hosts do not know an auditor's salt; for a sample nobody could have steered, take it from something fixed after delivery (a later block hash).

**Evidence.** `test/battery_audit.mjs` runs the join on the committed live runs (fly #101 against `M000`; the substrate against `registered-base.json` and, correctly failing, against the base row; the published wiring against the base row), names spoiled digests, swapped, duplicated and dropped runs, and checks the draw. With Python ≥ 3.10, numpy and pycryptodome (`FLYBNB_PYTHON`) it also runs the whole path on a synthetic brain: a founder recipe applied in place, the WASM kernel executing the battery into a worker-shaped archive, and `audit_reference.py` rebuilding and agreeing — and refusing a missing recipe, another battery file, a wrong `model_id` and the default unit. The full path on the real brain was run on fly #101's archived rows (task `0x4df627a8…7d32`): rebuilt to `model_id` `0x8af16bb9…`, 42/42.

**Limits.** The auditor trusts the archive's `modelId` as the fly's: that `modelId` is what the worker checked against the chain before it ran anything, and the archive's hash is what the job delivered on chain (`artifactHash`), but comparing the two is a chain read this offline audit does not do. Artifacts written before `baseModelId`/`deltaHash` were added to them find their recipe only when the fly is a committed founder (`flybnb/genesis/founder-profiles-v2.json`). A bred child's recipe is not on the founder mirror; supply it with `--recipes`.
