# AIGG Governance: Staked Voting, NFT Funding, and Project Returns
...
- Only staked AIGG has allocation voting power.
- Voting selects the destination of issuance; holding AIGG alone does not earn project returns.
- Returns belong to the staked voters supporting the relevant project, not all token holders or all voters across projects.
...
|---|---|
| AIGG issuance controller | Enforce the authorized supply schedule and per-epoch cap; mint only finalized allocations |
...
### 5.3 The Single-Subnet Phase

At launch, votes may support FlyBnB or leave funds unallocated. There is no competition among freely registered projects. The approved FlyBnB offer identifies the experiment batch and purchase terms; only one active offer per epoch is needed initially.
...
An approved offer binds: chain and collection, adapter version, accepted payment asset, unit or maximum total price, maximum quantity, experiment scope, delivery deadline, refund terms, NFT beneficiary rights, and the receiving cohort vault. The launch route purchases newly issued NFTs with explicit funding obligations. Secondary-market purchases are outside launch scope because their proceeds normally go to sellers rather than new research delivery.

**Proposed default:** mint AIGG only at a successful acquisition transaction, atomically with payment and verified NFT receipt. An unsuccessful transaction produces neither a lasting mint nor a purchase. Partial batches are allowed under the same price and quantity bounds; expired or unspent allocation remains unissued. Finalization and each acquisition use unique epoch/project/offer identifiers to prevent duplicate allocation or minting.

NFTs belong to the project/epoch vault on behalf of its beneficiary cohort. They are not personal assets of an operator or discretionary assets that later voters can redirect. Initial acquisition retains NFTs; active trading is not a launch feature. A future sale facility requires cohort terms defining authorization, minimum proceeds, fees, and allocation of sale proceeds before that facility is enabled.

Historical funding attribution identifies the AIGG allocation and cohort. It remains distinct from current NFT ownership, scientific authorship, and actual computation contributions.
...
| NFT sale proceeds, if later enabled | Separate capital recovery from realized gain; distribution follows original cohort sale terms |
...
**Proposed default:** the distributable NFT income belongs entirely to that project's cohort after collection-defined upstream splits and explicitly disclosed costs. There is no automatic diversion to all AIGG holders or a new treasury profit share. Operational costs should normally have a separate approved budget; an operator cannot invent deductions after voting.
...
|---|---|
| No eligible stake or quorum failure | No issuance; no project allocation |
| Nobody supports FlyBnB | No forced allocation to the sole admitted project |
| Purchase fails or deadline expires | Failed portion remains unissued; successful purchases remain attributed to their cohort |
...
| Alternative | Benefit | Reason not selected for launch |
|---|---|---|
...
| Rolling common treasury portfolio, income to current stakers | Simpler ongoing membership model | Lets later participants acquire exposure to earlier decisions and disconnects returns from the original project vote |
...
The design reuses NFT acquisition as the funding route. It does not require an alpha token per subnet, an AMM, token-price oracles, cross-chain settlement, transferable vote receipts, or a general NFT trading strategy at launch.

## 12. Delivery Stages

| Stage | Scope | Exit evidence |
|---|---|---|
| G0: Economic specification | Approve the parameter manifest and cohort rights; define FlyBnB offer and revenue terms | Worked accounting examples cover issuance, abstention, partial purchases, refunds, and delayed revenue |
...
| G2: Single-subnet governance | Stake checkpoints, epochs, FlyBnB-only acquisition, vault custody, claims | One complete controlled cycle from stake through NFT purchase, verified task, receipt, and correct historical-voter payout |
...
4. Zero votes, failed quorum, and abstentions produce the specified unissued balance.
5. Total minted AIGG never exceeds finalized allocations or global authorization; finalization and purchase retries cannot double mint.
...
|---|---|
| Token supply and mint authority | Publish supply ceiling, initial distribution, epoch caps, and exact authorized minters; no numeric tokenomics assumed |
...
| Historical reward rights | Approve the cohort model in Section 7, including rights after stake withdrawal and any eventual portfolio liquidation |
