...
- [ ] Configure Render relayer and gateway consistently, preserve credentials and set the approved public RPC, publish main configuration and records. Enable new inventory only after verification. Verify familyHosting, capacity wiring, Adopt and Host UI.
- [ ] Document old bond withdrawal/new bond and capacity opt-in; no automatic movement of user funds. Record any unresolved legacy obligations explicitly.
