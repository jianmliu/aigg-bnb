...
`artifactHash` is an operator publication attestation gated on chain finality, not an on-chain proof of data availability or of every scientific readout. Clients can fetch the artifact, check its hash and independently replay it. The worker's replay runs the same WASM kernel as the hosts, so it cannot catch a bug every copy of that kernel shares; `battery/audit.mjs` audits a sample of archived artifacts against numpy's `flybnb/analysis/intlif.py`, rebuilding each payload from its base and recipe (see `flybnb/battery/README.md`, *Auditing the production pipeline*). Artifacts record the individual's `baseModelId` and `deltaHash` so that the recipe can be found by content address. A malicious or unavailable operator can delay delivery; unused funds are recoverable subject to the conditions below.
