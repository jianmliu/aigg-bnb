...
4. ~~Whether the token owner's share of task fees is a protocol rule or a social one.~~ A protocol rule, in two
   ...
   decide is the number: `ROYALTY_BPS` is immutable and inside every id, and upstream's bound applies -- a royalty-free
   twin of the same bytes is one call away, so the rate has to cost a client less than standing up that twin's
   executors would.
