...
### Real callback acceptance

The staged mesh subsequently passed a real Chainlink callback and direct on-chain execution run. See [the acceptance report](VRF_ACCEPTANCE_2026-09-22.md). The request remained pending with the initial 0.01 tBNB balance and fulfilled shortly after a 0.02 tBNB top-up, consistent with the maximum-gas-price balance check; the original request was preserved and actual VRF cost was 0.0002371824 tBNB. This supersedes the staged-deployment statement above that no callback had yet been verified. Browser/gateway cutover and NFT migration remain separate.
