...
async function plan(body, { signal, onCold, onCapacity } = {}) {
  ...
  const fee = BigInt(steps) * BigInt(redundancy) * cfg.weiPerStep;
...
async function respond(req, res, body) {
  ...
  const cleanup = () => { clearInterval(alive); if (listener) busRef.off("event", listener); };
  ...
  try {
    ...
    const c = create(p, body); start(c, body.background ? undefined : abort.signal);
    if (body.background) return json(res, 200, view(c));
    if (!body.stream) { await done(c); if (!res.destroyed) return c.status === "completed" ? json(res, 200, view(c)) : json(res, c.error.status, { error: c.error, receipt: c.receipt, id: c.id }); return; }
    open(); busRef = bus.get(c.id);
    listener = (ev) => {
      if (ev.type === "response.in_progress") write(ev.type, { response: view(c), executor: ev.executor, result: c.results[ev.executor] });
      else write(ev.type, { response: view(c) });
      if (ev.type === "response.completed" || ev.type === "response.failed") { cleanup(); res.end(); }
    };
    for (const ev of c.events) listener(ev);
    if (!res.writableEnded && !res.destroyed) busRef.on("event", listener);
  } catch (error) {
